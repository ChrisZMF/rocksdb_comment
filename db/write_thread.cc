...
//goal_mask = STATE_GROUP_LEADER | STATE_MEMTABLE_WRITER_LEADER | STATE_PARALLEL_MEMTABLE_WRITER | STATE_COMPLETED
uint8_t WriteThread::AwaitState(Writer* w, uint8_t goal_mask,
  ...
  for (uint32_t tries = 0; tries < 200; ++tries)    
  {
    ...
    if ((state & goal_mask) != 0) { //当状态发生改变则停止等待，返回新状态
      return state;
    }
    port::AsmVolatilePause();   //忙等状态，等待1ms，适配不同操作系统;共等待200ms
  }
...
bool WriteThread::LinkOne(Writer* w, std::atomic<Writer*>* newest_writer) {
  ...
  Writer* writers = newest_writer->load(std::memory_order_relaxed); //原子的获取 newest_writer队列的地址
  while (true) {
    // If write stall in effect, and w->no_slowdown is not true,
    // block here until stall is cleared. If its true, then return
    // immediately
    if (writers == &write_stall_dummy_) { //写阻塞，且没有slowdown的写操作直接则返回，无则阻塞在此等到stall结束， no_slowdown机制？
      if (w->no_slowdown) {   //1、no_slowdown则返回，并设置状态为STATE_COMPLETED
      ...
      // stall clearing
      {
        ...
        if (writers == &write_stall_dummy_) {
          stall_cv_.Wait();   //2、无slowdown则阻塞等待
    ...
    w->link_older = writers;  //将w指向newest_writer_
    if (newest_writer->compare_exchange_weak(writers, w)) { //交换链表头，w作为表头
...
void WriteThread::JoinBatchGroup(Writer* w) {
  ...
  bool linked_as_leader = LinkOne(w, &newest_writer_);  // WriteThread： std::atomic<Writer*> newest_writer_;

  if (linked_as_leader) {
    SetState(w, STATE_GROUP_LEADER);  //1、STATE_GROUP_LEADER，设置w为leader
  }

  TEST_SYNC_POINT_CALLBACK("WriteThread::JoinBatchGroup:Wait", w);

  if (!linked_as_leader) {  //2、如果不是leader，则等待，直到以下所述3种情况出现
    /**
     * Wait util:
    ...
    AwaitState(w, STATE_GROUP_LEADER | STATE_MEMTABLE_WRITER_LEADER |
               ...
               &jbg_ctx);     //等待直到state变成以上几种之一则退出等待
...
size_t WriteThread::EnterAsBatchGroupLeader(Writer* leader,
  ...
  write_group->size = 1;
  Writer* newest_writer = newest_writer_.load(std::memory_order_acquire);   //找到writer group
  ...
  while (w != newest_writer) {
    ...
    size += batch_size; //将writers的batch组合起来
...
bool WriteThread::CompleteParallelMemTableWriter(Writer* w) {
  ...
  if (write_group->running-- > 1) { //如果不是最后一个，则等待，且返回false，不能退出write group；running递减
    // we're not the last one
    AwaitState(w, STATE_COMPLETED, &cpmtw_ctx); //阻塞直到所有的writer都STATE_COMPLETED
    return false;
  }
...
void WriteThread::ExitAsBatchGroupLeader(WriteGroup& write_group,
  ...
  if (enable_pipelined_write_) {
    ...
    if (head != last_writer ||
        !newest_writer_.compare_exchange_strong(head, nullptr)) {   //找到新的writer，更新为leader
    ...
    while (last_writer != leader) { //更新group中writer的state为STATE_COMPLETED
      assert(last_writer);
