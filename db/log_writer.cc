...
IOStatus Writer::AddRecord(const Slice& slice) {    //写入record信息到wal文件,入参为WriteBatch::req_
  ...
  do {
    ...
    if (leftover < header_size) {
      ...
      if (leftover > 0) {
        ...
        assert(header_size <= 11);  //block未填满的部分用x00填充
        s = dest_->Append(Slice("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
                                static_cast<size_t>(leftover)));    //append中会调用分配内存大小，内存大小限制，在DBImpl::CreateWAL中SetPreallocationBlockSize()设置
        if (!s.ok()) {
          break;
        }
      }
    ...
    s = EmitPhysicalRecord(type, ptr, fragment_length); //ptr为WriteBatch::req_的数据部分
    ptr += fragment_length;
...
IOStatus Writer::EmitPhysicalRecord(RecordType t, const char* ptr, size_t n) {  //ptr即为数据
