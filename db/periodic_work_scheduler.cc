...
void PeriodicWorkScheduler::Register(DBImpl* dbi,
                                     ...
                                     unsigned int stats_persist_period_sec) { //参数1是DB实例，此函数注册三个任务，dumpstats、persiststats、flushinfolog
  static std::atomic<uint64_t> initial_delay(0);
  timer->Start();   //启动timer周期任务，在start中通过thread.reset()来运行timer.run()，run()中依次运行timer->Add()添加的任务接口
