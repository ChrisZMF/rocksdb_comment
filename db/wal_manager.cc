...
Status WalManager::GetSortedWalFiles(VectorLogPtr& files) {
  ...
  VectorLogPtr logs;
  //1. 找到db目录下的wal文件并排序保存在出参logs
  //2. 每个文件都创建LogFileImpl
  //3. db_options_.wal_dir就是db目录; 如：./dbdir/db ./dbdir/db/archive
  ...
  std::string archivedir = ArchivalDirectory(db_options_.wal_dir);  //构造archivedir
  Status exists = env_->FileExists(archivedir);
  if (exists.ok()) {
    s = GetSortedWalsOfType(archivedir, files, kArchivedLogFile); //读取archive目录下的所有wal文件,并排序保存在files中
    if (!s.ok()) {
      return s;
    }
  ...
  for (auto& log : logs) {
    if (log->LogNumber() > latest_archived_log_number) {
      files.push_back(std::move(log));  //db目录下的wal文件拼接到files中汇总，并传出
    } else {
...
Status WalManager::GetUpdatesSince(
  ...
  Status s = GetSortedWalFiles(*wal_files); //找到所有排好序的wal文件，并排序所有wal文件，按文件名排序（从老到新）
  if (!s.ok()) {
    return s;
  }

  s = RetainProbableWalFiles(*wal_files, seq);  //二分查找依次迭代文件，找到>=seq对应的文件，删除seq之前的所有文件信息，剩余文件用于迭代
  if (!s.ok()) {
    return s;
  }
  //构造iter返回给用户;（std::move不移动任何东西，只是将左值引用转换为右值引用，以用于移动语句）
  iter->reset(new TransactionLogIteratorImpl( //reset销毁当前的指针内容，重设为新的指针
...
void WalManager::ArchiveWALFile(const std::string& fname, uint64_t number) {  //WAL 文件归档
...
Status WalManager::GetSortedWalsOfType(const std::string& path,
  ...
  const Status status = env_->GetChildren(path, &all_files);  //读出db目录下的所有文件到all_files
  if (!status.ok()) {
    return status;
  }
  ...
  for (const auto& f : all_files) {
    uint64_t number;
    FileType type;
    if (ParseFileName(f, &number, &type) && type == kWalFile) { //找出wal文件，获取其number
      SequenceNumber sequence;
      Status s = ReadFirstRecord(log_type, number, &sequence);  //读取wal第一个有效record，解析出start_sequence
      if (!s.ok()) {
        return s;
      }
      if (sequence == 0) {
      ...
      log_files.push_back(std::unique_ptr<LogFile>(
          new LogFileImpl(number, log_type, sequence, size_bytes)));  //创建logfile对象，都存入log_files，故start_sequence均已知
    }
  }
  std::sort(  //将所有wal文件排序log_files，出参log_files
...
Status WalManager::ReadFirstRecord(const WalFileType type,
  ...
  *sequence = 0;
  ...
  if (type == kAliveLogFile) {
    std::string fname = LogFileName(db_options_.wal_dir, number); //构造wal文件名，number就是wal文件名的前缀
