    ...
    for (size_t i = 0; i < storage_info_.files_[level].size(); i++) {
      ...
      if (f->refs <= 0) {
        ...
        vset_->obsolete_files_.push_back(
            ObsoleteFileInfo(f, cfd_->ioptions()->cf_paths[path_id].path)); //ref为0的sst文件放到obsolete_files_等待删除
      }
    }
  }
}
...
class LevelIterator final : public InternalIterator {
 public:
  ...
  LevelIterator(TableCache* table_cache, const ReadOptions& read_options,
                ...
                const LevelFilesBrief* flevel,  //某层level的文件信息，包含文件数量，文件信息等
...
void LevelIterator::Seek(const Slice& target) {
  ...
  if (need_to_reseek) {
    ...
    InitFileIterator(new_file_index); //如果没有初始化file_iter_，则在此初始化，底层创建sst文件的iter，跟L0接口相同
  }
...
void LevelIterator::SeekForPrev(const Slice& target) {
  ...
  if (new_file_index >= flevel_->num_files) {
    new_file_index = flevel_->num_files - 1;  //迭代文件
  }
...
bool LevelIterator::SkipEmptyFileForward() {
  ...
  while (file_iter_.iter() == nullptr ||
    ...
    InitFileIterator(file_index_ + 1);  //文件索引+1，依次遍历level中的所有文件；创建sst文件的iter
    if (file_iter_.iter() != nullptr) {
      file_iter_.SeekToFirst();
    }
  }
...
void LevelIterator::SetFileIterator(InternalIterator* iter) {
  ...
  InternalIterator* old_iter = file_iter_.Set(iter);  //file_iter_赋值
...
void LevelIterator::InitFileIterator(size_t new_file_index) {
  if (new_file_index >= flevel_->num_files) {
    ...
    if (file_iter_.iter() != nullptr && !file_iter_.status().IsIncomplete() &&
      ...
      file_index_ = new_file_index;
      InternalIterator* iter = NewFileIterator(); //创建sst文件的iter
...
void Version::AddIteratorsForLevel(const ReadOptions& read_options,
  ...
  if (level == 0) { //L0层创建iter
    // Merge all level zero files together since they may overlap
    for (size_t i = 0; i < storage_info_.LevelFilesBrief(0).num_files; i++) {
      const auto& file = storage_info_.LevelFilesBrief(0).files[i]; //逐个文件创建iter
      merge_iter_builder->AddIterator(cfd_->table_cache()->NewIterator( //创建L0层table_cache的iter
  ...
  } else if (storage_info_.LevelFilesBrief(level).num_files > 0) {    //Ln层创建iter
    // For levels > 0, we can use a concatenating iterator that sequentially
    // walks through the non-overlapping files in the level, opening them
    // lazily.
    auto* mem = arena->AllocateAligned(sizeof(LevelIterator));
    merge_iter_builder->AddIterator(new (mem) LevelIterator(      //创建Ln层LevelIterator
...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
  ...
  Slice ikey = k.internal_key();  //key在内部的形式
  Slice user_key = k.user_key();  //user传入的key，含有timestamp信息？
  ...
  if (merge_operator_) {  //如果是merge操作，则pin住blocks
  ...
  while (f != nullptr) {
    ...
    *status = table_cache_->Get(    //1、读table_cache
    ...
    switch (get_context.State()) {
      ...
      case GetContext::kMerge:
        ...
        break;
      case GetContext::kFound:    //sst文件中找到了key信息
        if (fp.GetHitFileLevel() == 0) {  //记录sst文件所属的level
        ...
        if (is_blob_index) {    //blob是什么？？是titandb的kv分离的blob文件
          if (do_merge && value) {
...
void VersionStorageInfo::ComputeCompactionScore(
  ...
  for (int level = 0; level <= MaxInputLevel(); level++) {
    ...
    if (level == 0) { //L0层
      ...
      for (auto f : files_[level]) {
        if (!f->being_compacted) {  //排除正在支持compaction的文件
  ...
  for (int i = 0; i < num_levels() - 2; i++) {      //将各层按score从大到小排序，不包含最后一层
...
Status VersionSet::ListColumnFamilies(std::vector<std::string>* column_families,
  ...
  uint64_t manifest_file_number;
  Status s =
      GetCurrentManifestPath(dbname, fs, &manifest_path, &manifest_file_number);  //找出db目录的manifest文件路径、数量
  if (!s.ok()) {
    return s;
  }
