...
bool Reader::ReadRecord(Slice* record, std::string* scratch,
                        WALRecoveryMode wal_recovery_mode) {  //只有读到完整的record才会返回，一个record可能大于32k
  scratch->clear();
  record->clear();
  bool in_fragmented_record = false;
  // Record offset of the logical record that we're reading
  // 0 is a dummy value to make compilers happy
  uint64_t prospective_record_offset = 0; //记录读取的off

  Slice fragment;
  while (true) {
    uint64_t physical_record_offset = end_of_buffer_offset_ - buffer_.size(); //buffer_是怎么来的？一开始为空，在ReadPhysicalRecord中判断，并读入数据
    size_t drop_size = 0;
    const unsigned int record_type = ReadPhysicalRecord(&fragment, &drop_size); //取出一条record的物理块（可能record>32k被切割），实际就是一次写入的batch存到fragment，返回type
    switch (record_type) {  //根据type进行相应的处理
      case kFullType:
      case kRecyclableFullType:
        ...
        scratch->clear();
        *record = fragment; //record出参，传出本次读取的record中的batch信息，可保证读出的batch是完整的
      ...
      case kRecyclableFirstType:
        ...
        prospective_record_offset = physical_record_offset;
        scratch->assign(fragment.data(), fragment.size());  //如果是fragement则放到scratch
        in_fragmented_record = true;
        break;

      case kMiddleType:
      case kRecyclableMiddleType:
        if (!in_fragmented_record) {  //缺失头部的report片段会被丢弃，报告drop信息
      ...
      case kRecyclableLastType:
        if (!in_fragmented_record) {  //如果文件头的record是一个被截断的后半部分，没有第一部分信息，则记录drop掉，fragment会在下一轮循环ReadPhysicalRecord被重新赋值
          ReportCorruption(fragment.size(),
                           "missing start of fragmented record(2)");
        } else {
          scratch->append(fragment.data(), fragment.size());  //scratch依次读取被切割的record的各部分，组织起来
          *record = Slice(*scratch);  //最终将完整的record放到record返回给调用方
          last_record_offset_ = prospective_record_offset;
          return true;
        }
        break;
...
bool Reader::ReadMore(size_t* drop_size, int *error) {
  if (!eof_ && !read_error_) {
    ...
    Status status = file_->Read(kBlockSize, &buffer_, backing_store_);  //一次从文件中读取一个kBlockSize(32k)的大小，存到buffer_中，buffer_一开始为空
...
unsigned int Reader::ReadPhysicalRecord(Slice* result, size_t* drop_size) {
  while (true) {
    ...
    if (buffer_.size() < static_cast<size_t>(kHeaderSize)) {
      ...
      // not be used anyway
      int r = kEof;
      if (!ReadMore(drop_size, &r)) { //buffer_一开始为空，发现buffer_内容不足，此处从文件中读取32k数据到buffer_
        return r;
      }
      continue;
    }
    ...
    const uint32_t length = a | (b << 8); //从wal的record头部取出length值，也就是payload的length
    ...
    // Check crc
    if (checksum_) {  //wal记录的头部的前4字节为crc，内容校验
      uint32_t expected_crc = crc32c::Unmask(DecodeFixed32(header));
      uint32_t actual_crc = crc32c::Value(header + 6, length + header_size - 6);  //从type开始计算crc
    ...
    buffer_.remove_prefix(header_size + length);  //从buf中移除掉已读取的内容

    *result = Slice(header + header_size, length);  //去掉record head(4+2+1的crc+len+type)的内容作为result的返回，也就是batch的内容
    return type;
  }
}
