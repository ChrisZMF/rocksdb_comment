...
Status WriteBatch::Iterate(Handler* handler) const {
  if (rep_.size() < WriteBatchInternal::kHeader) {  //说明无数据
...
//WriteBatch迭代器，迭代一遍WriteBatch的内容，找出各个段的内容，执行相应的操作，如handler->PutCF
Status WriteBatchInternal::Iterate(const WriteBatch* wb,
                                   WriteBatch::Handler* handler, size_t begin,  //begin为kHeader后存数据的头
                                   size_t end) {  //end为req_.size()
  if (begin > wb->rep_.size() || end > wb->rep_.size() || end < begin) {
    return Status::Corruption("Invalid start/end bounds for Iterate");
  }
  assert(begin <= end);
  Slice input(wb->rep_.data() + begin, static_cast<size_t>(end - begin)); //batch中的数据部分构造为slice，剔除kHeader头
  bool whole_batch =
  ...
  while (((s.ok() && !input.empty()) || UNLIKELY(s.IsTryAgain()))) {
    ...
    if (LIKELY(!s.IsTryAgain())) {
      ...
      s = ReadRecordFromWriteBatch(&input, &tag, &column_family, &key, &value,
                                   &blob, &xid);  //取出batch数的tag、column_family、key、value；并移除input中已读的部分
      if (!s.ok()) {
        return s;
      }
    }
    ...
    switch (tag) {
      ...
      case kTypeBlobIndex:
        ...
        break;
      case kTypeLogData:
        handler->LogData(blob); //LogData被忽略
...
uint32_t WriteBatchInternal::Count(const WriteBatch* b) {
  return DecodeFixed32(b->rep_.data() + 8); //data+8就是预留4字节的count的位置，一开始为空，则为0
}
...
Status WriteBatchInternal::Put(WriteBatch* b, uint32_t column_family_id,
  ...
  if (key.size() > size_t{port::kMaxUint32}) {  //分别检查key、value的size
  ...
  LocalSavePoint save(b);  //一开始创建的savepoint_为空
  WriteBatchInternal::SetCount(b, WriteBatchInternal::Count(b) + 1);//WriteBatch::rep_设置预留的count的位置的值
  if (column_family_id == 0) {  //填入当前batch的type
  ...
  if (0 == b->timestamp_size_) {
    PutLengthPrefixedSlice(&b->rep_, key);  //填入key
  } else {
    PutVarint32(&b->rep_,
                static_cast<uint32_t>(key.size() + b->timestamp_size_));
    b->rep_.append(key.data(), key.size());
    b->rep_.append(b->timestamp_size_, '\0'); //占位的timestamp位置填0
  }
  PutLengthPrefixedSlice(&b->rep_, value);  //填入value
  b->content_flags_.store(
      b->content_flags_.load(std::memory_order_relaxed) | ContentFlags::HAS_PUT,
      std::memory_order_relaxed);
  return save.commit(); //save由b来初始化，默认直接返回OK状态，b.max_bytes_默认为0
}
...
Status WriteBatchInternal::Put(WriteBatch* b, uint32_t column_family_id,
  ...
  Status s = CheckSlicePartsLength(key, value); //检查key、value的size是否正确
  if (!s.ok()) {
    return s;
  }
  ...
  if (0 == b->timestamp_size_) {
    PutLengthPrefixedSliceParts(&b->rep_, key); //填入key
  } else {
    PutLengthPrefixedSlicePartsWithPadding(&b->rep_, key, b->timestamp_size_);
  }
  PutLengthPrefixedSliceParts(&b->rep_, value); //填入value
  b->content_flags_.store(
      b->content_flags_.load(std::memory_order_relaxed) | ContentFlags::HAS_PUT,
      std::memory_order_relaxed);
  return save.commit(); //save由b来初始化，正常时直接返回OK状态
}
...
class MemTableInserter : public WriteBatch::Handler {
 ...
 public:
  ...
  Status PutCFImpl(uint32_t column_family_id, const Slice& key,
    ...
    Status ret_status;
    if (UNLIKELY(!SeekToColumnFamily(column_family_id, &ret_status))) { //查找CF失败
    ...
    MemTable* mem = cf_mems_->GetMemTable();  //找到对应的memtable
    auto* moptions = mem->GetImmutableMemTableOptions();  //Immemtable的options
    ...
    if (!moptions->inplace_update_support) {
      ret_status =
          mem->Add(sequence_, value_type, key, value,
                   concurrent_memtable_writes_, get_post_process_info(mem), //如果开启了并发写memtable则concurrent_memtable_writes为true
...
Status WriteBatchInternal::InsertInto(
  ...
  for (auto w : write_group) {
    ...
    w->sequence = inserter.sequence();  //写memtable的时候，多个batch的sequence都是用最新的一个
    ...
    SetSequence(w->batch, inserter.sequence());   //设置batch的seq
    inserter.set_log_number_ref(w->log_ref);
    w->status = w->batch->Iterate(&inserter);   //迭代batch写入数据
    if (!w->status.ok()) {
      return w->status;
    }
...
Status WriteBatchInternal::InsertInto(
  ...
  MemTableInserter inserter(
      ...
      concurrent_memtable_writes, nullptr /*has_valid_writes*/, seq_per_batch,  //如果开启了并发写memtable则concurrent_memtable_writes为true
      batch_per_txn, hint_per_batch);
  SetSequence(writer->batch, sequence);
  inserter.set_log_number_ref(writer->log_ref); // log_ref is log number that memtable insert should reference,memtable后续刷入的文件？
...
Status WriteBatchInternal::SetContents(WriteBatch* b, const Slice& contents) {
  assert(contents.size() >= WriteBatchInternal::kHeader); //kHeader是12，表示internalBatch的头部8字节的seq，4字节的count
...
Status WriteBatchInternal::Append(WriteBatch* dst, const WriteBatch* src,
  ...
  dst->rep_.append(src->rep_.data() + WriteBatchInternal::kHeader, src_len);//src_len只有数据部分，不含kHeader头，rep_是std::string类型
