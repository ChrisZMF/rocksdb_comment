...
class LocalSavePoint {
 public:
  ...
  Status commit() {
    ...
    committed_ = true;
#endif
    if (batch_->max_bytes_ && batch_->rep_.size() > batch_->max_bytes_) { //1、当大小超限后,默认max_bytes_为0，也就是不加限制
      batch_->rep_.resize(savepoint_.size); //resize裁掉加入的数据，回复req_为初始大小
      WriteBatchInternal::SetCount(batch_, savepoint_.count);
      batch_->content_flags_.store(savepoint_.content_flags,
                                   std::memory_order_relaxed);
      return Status::MemoryLimit();
    }
    return Status::OK();  //2、正常情况直接返回OK
  }
