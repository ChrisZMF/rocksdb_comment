...
extern Status ParseInternalKey(const Slice& internal_key, //解析InternalKey，ParsedInternalKey含有sequence成员
                               ParsedInternalKey* result, bool log_err_key);

// Returns the user key portion of an internal key.
inline Slice ExtractUserKey(const Slice& internal_key) {
  assert(internal_key.size() >= kNumInternalBytes);
  return Slice(internal_key.data(), internal_key.size() - kNumInternalBytes); //kNumInternalBytes=8,去掉key尾部的sequence|type的8字节
}
...
class InternalKey {
 ...
 public:
  ...
  InternalKey(const Slice& _user_key, SequenceNumber s, ValueType t) {
    AppendInternalKey(&rep_, ParsedInternalKey(_user_key, s, t)); //rep_构造为：userkey|seq+type,其中seq+type构造为一个uint64也就是8字节，type表示操作类型，如：kTypeValue、kTypeRangeDeletion
  }
  ...
  void SetMinPossibleForUserKey(const Slice& _user_key) {
    AppendInternalKey(&rep_, ParsedInternalKey(_user_key, kMaxSequenceNumber,
                                               kValueTypeForSeek)); //rep_构造为：userkey|seq+type,其中seq+type构造为一个uint64也就是8字节
  }
  ...
  Slice user_key() const { return ExtractUserKey(rep_); } //由于internalkey头部是usekey，后跟8字节seq+type，故返回其头部即可
