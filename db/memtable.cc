...
Status MemTable::Add(SequenceNumber s, ValueType type,  //type是WriteBatch的type
  ...
  const uint32_t encoded_len = VarintLength(internal_key_size) +
                               ...
                               val_size;  //将k-v编码，编码后size
  char* buf = nullptr;
  std::unique_ptr<MemTableRep>& table =
      type == kTypeRangeDeletion ? range_del_table_ : table_;
  KeyHandle handle = table->Allocate(encoded_len, &buf);  //KeyHandle就是void *，也就是一个指针，类似malloc分配内存返回的指针

  char* p = EncodeVarint32(buf, internal_key_size);
  memcpy(p, key.data(), key_size);
  Slice key_slice(p, key_size);
  p += key_size;
  uint64_t packed = PackSequenceAndType(s, type); //将sequence和type打成一个pack，size为8字节，低8位为type
  EncodeFixed64(p, packed);
  p += 8;
  p = EncodeVarint32(p, val_size);
  memcpy(p, value.data(), val_size);
  assert((unsigned)(p + val_size - buf) == (unsigned)encoded_len); //encode后格式|4字节keysize+8|keysize的key|8字节的sequence+type|4字节valsize|value|
  size_t ts_sz = GetInternalKeyComparator().user_comparator()->timestamp_size();
  Slice key_without_ts = StripTimestampFromUserKey(key, ts_sz);

  if (!allow_concurrent) {  //不允许并发写memtable
    // Extract prefix for insert with hint.
    if (insert_with_hint_prefix_extractor_ != nullptr &&
        insert_with_hint_prefix_extractor_->InDomain(key_slice)) {
      Slice prefix = insert_with_hint_prefix_extractor_->Transform(key_slice);
      bool res = table->InsertKeyWithHint(handle, &insert_hints_[prefix]);  //见skiplistrep.cc的InsertKeyWithHint；由于memtable是skiplist，且class SkipListRep : public MemTableRep
      if (UNLIKELY(!res)) {
...
bool MemTable::Get(const LookupKey& key, std::string* value,
  ...
  size_t ts_sz = GetInternalKeyComparator().user_comparator()->timestamp_size();
  Slice user_key_without_ts = StripTimestampFromUserKey(key.user_key(), ts_sz); //取出要查找的key，去除timestamp
  //1、bloom过滤判断key是否可能存在
  if (bloom_filter_) {
    // when both memtable_whole_key_filtering and prefix_extractor_ are set,
    // only do whole key filtering for Get() to save CPU
    if (moptions_.memtable_whole_key_filtering) {
      may_contain = bloom_filter_->MayContain(user_key_without_ts); //bloom过滤器判断
    } else {
...
void MemTable::GetFromTable(const LookupKey& key,
  ...
  table_->Get(key, &saver, SaveValue);    //此处Get函数为MemTableRep::Get()
  *seq = saver.seq;
}
