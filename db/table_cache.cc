...
InternalIterator* TableCache::NewIterator(
  ...
  table_reader = fd.table_reader; //每个文件独立的table_reader
  ...
  if (s.ok()) {
    if (options.table_filter &&
      ...
      result = table_reader->NewIterator(options, prefix_extractor, arena,
                                   ...
                                   allow_unprepared_value);   //实际创建iter
    }
    if (handle != nullptr) {
...
Status TableCache::Get(const ReadOptions& options,
                       ...
                       GetContext* get_context, //本结构里存了出参value的地址，查找结果传入其对应元素即可
                       const SliceTransform* prefix_extractor,
                       HistogramImpl* file_read_hist, bool skip_filters,
  ...
  if (ioptions_.row_cache && !get_context->NeedToReadSequence()) {
    ...
    CreateRowCacheKeyPrefix(options, fd, k, get_context, row_cache_key);  //构造key前缀
    done = GetFromRowCache(user_key, row_cache_key, row_cache_key.Size(),
                           get_context);    //1、从cache中读取
    if (!done) {
  ...
  TableReader* t = fd.table_reader;
  Cache::Handle* handle = nullptr;
                                //2、如果cache中没有找到，则从sst文件中查找
  if (!done) {
    assert(s.ok());
    if (t == nullptr) {  //2.1、查找sst文件
      s = FindTable(options, file_options_, internal_comparator, fd, &handle,
                    prefix_extractor,
                    options.read_tier == kBlockCacheTier /* no_io */,
    ...
    if (s.ok()) {
      ...
      s = t->Get(options, k, get_context, prefix_extractor, skip_filters);    //2.2、具体读取sst文件的操作
