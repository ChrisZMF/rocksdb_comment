...
void LevelCompactionBuilder::SetupInitialFiles() {
  ...
  for (int i = 0; i < compaction_picker_->NumberLevels() - 1; i++) {  //1. pick除最后一层的文件
    start_level_score_ = vstorage_->CompactionScore(i);
    start_level_ = vstorage_->CompactionScoreLevel(i);
    assert(i == 0 || start_level_score_ <= vstorage_->CompactionScore(i - 1));  //下层的score一定<=上层？
  ...
  compaction_picker_->PickFilesMarkedForCompaction(
      cf_name_, vstorage_, &start_level_, &output_level_, &start_level_inputs_);  //2.pick被标记为需要compaction的文件
  if (!start_level_inputs_.empty()) {
  ...
  PickFileToCompact(vstorage_->BottommostFilesMarkedForCompaction(), false);  //3. pick最后一层中需要compaction的文件
  if (!start_level_inputs_.empty()) {
  ...
  // TTL Compaction
  PickFileToCompact(vstorage_->ExpiredTtlFiles(), true);  //4. pick过期的的文件
  if (!start_level_inputs_.empty()) {
  ...
  // Periodic Compaction
  PickFileToCompact(vstorage_->FilesMarkedForPeriodicCompaction(), false);  //5. pick被标记为定期compaction的文件
  if (!start_level_inputs_.empty()) {
