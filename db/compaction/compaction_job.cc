  ...
  if (c->ShouldFormSubcompactions()) {
    {
      ...
      GenSubcompactionBoundaries(); //拆分每个subcompaction的边界
    }
...
Status CompactionJob::Run() {
  ...
  for (size_t i = 1; i < compact_->sub_compact_states.size(); i++) {
    thread_pool.emplace_back(&CompactionJob::ProcessKeyValueCompaction, this,
                             &compact_->sub_compact_states[i]); //并发执行从第二个subcompaction起的所有subcompaction任务
  }

  // Always schedule the first subcompaction (whether or not there are also
  // others) in the current thread to be efficient with resources
  ProcessKeyValueCompaction(&compact_->sub_compact_states[0]);  //执行keyvalue的compaction

  // Wait for all other threads (if there are any) to finish execution
  for (auto& thread : thread_pool) {  //等待compaction结束
    thread.join();
  }
