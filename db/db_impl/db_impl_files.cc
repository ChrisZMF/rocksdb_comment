...
Status DBImpl::SetDBId() {
  ...
  if (db_id_.empty()) {
    ...
    // file or Manifest.
    if (s.IsNotFound()) {
      s = SetIdentityFile(env_, dbname_); //如果没有 IDENTYFY文件则跟else走一样的流程，生成该文件，并生成新的dbid
      if (!s.ok()) {
        return s;
      }
    } else if (!s.ok()) {
    ...
    s = GetDbIdentityFromIdentityFile(&db_id_); //从dbname/IDENTITY 文件中获取 db_id_ 信息
    if (immutable_db_options_.write_dbid_to_manifest && s.ok()) {
      VersionEdit edit;
      edit.SetDBId(db_id_); //设置dbid
      Options options;
    ...
    s = SetIdentityFile(env_, dbname_, db_id_);   //db_id_不为空，则创建 dbname/IDENTITY 文件，保存dbid
  }
  return s;
}
