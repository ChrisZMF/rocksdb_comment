...
DBOptions SanitizeOptions(const std::string& dbname, const DBOptions& src) {
  ...
  if (result.wal_dir.empty()) {
    ...
    result.wal_dir = dbname;  //即 DIR/db
  }
...
Status DBImpl::NewDB(std::vector<std::string>* new_filenames) {
  VersionEdit new_db;
  Status s = SetIdentityFile(env_, dbname_);  //生成DB的IDENTYFY文件
  if (!s.ok()) {
    return s;
  }
  ...
  if (immutable_db_options_.write_dbid_to_manifest) {
    ...
    new_db.SetDBId(temp_db_id); //创建DB_id
  }

  new_db.SetLogNumber(0);
  new_db.SetNextFile(2);
  new_db.SetLastSequence(0);  //sequence置0

  ROCKS_LOG_INFO(immutable_db_options_.info_log, "Creating manifest 1 \n");
  const std::string manifest = DescriptorFileName(dbname_, 1);  //创建manifest文件

  {
    ...
    std::string record;
    new_db.EncodeTo(&record); //构建DB的信息为一条record格式，准备写入manifest文件，包括DBid、sequence等信息
    s = log.AddRecord(record);  //写入manifest文件，新DB有关的信息，如sequence、filenum等
    if (s.ok()) {
      s = SyncManifest(env_, &immutable_db_options_, log.file()); //sync文件写入磁盘
    }
  }

  if (s.ok()) {
    // Make "CURRENT" file that points to the new manifest file.
    s = SetCurrentFile(fs_.get(), dbname_, 1, directories_.GetDbDir());   //写manifest成功，创建CURRENT文件记录最新的manifest文件
    if (new_filenames) {
...
Status DBImpl::Recover(
  ...
  std::vector<std::string> files_in_dbname; //dbname是db的路径
  if (!read_only) {         //false输入,获取文件路径
    ...
    s = env_->LockFile(LockFileName(dbname_), &db_lock_); //构造LOCK文件名称，并lock（暂不支持）？
    if (!s.ok()) {
      return s;
    }

    std::string current_fname = CurrentFileName(dbname_);     //构造current文件的名称
    ...
    else
    {
      ...
      for (const std::string& file : files_in_dbname) {
        ...
        if (ParseFileName(file, &number, &type) && type == kDescriptorFile) {
          ...
          s = Status::OK();
          manifest_path = dbname_ + "/" + file;   //找到manifest文件路径
          break;
        }
      }
    }

    if (s.IsNotFound()) { //没有找到manifest文件
      if (immutable_db_options_.create_if_missing) {
        s = NewDB(&files_in_dbname);  //创建新DB的versionEdit，写入创建新manifest文件、current文件
        is_new_db = true;
        if (!s.ok()) {
          return s;
        }
  ...
  else if (immutable_db_options_.best_efforts_recovery) { //如果是readonly打开
  ...
  if (!immutable_db_options_.best_efforts_recovery) { //条件含义？？
    s = versions_->Recover(column_families, read_only, &db_id_);  //从manifest文件中recover version信息？
  }
  else
  {
    assert(!files_in_dbname.empty());
    s = versions_->TryRecover(column_families, read_only, files_in_dbname,
                              &db_id_, &missing_table_file);  //从manifest文件中recover version
    if (s.ok()) {
  ...
  s = SetDBId();  //从 IDENTITY 文件中读取，或新生成 db_id 写入IDENTITY文件中
  if (s.ok() && !read_only) {
  ...
  if (s.ok() && !read_only) {
    ...
    for (auto cfd : *versions_->GetColumnFamilySet()) {
      s = cfd->AddDirectories(&created_dirs);   //创建CF的文件夹
      if (!s.ok()) {
        return s;
      }
    }
 ...
 //recover WAL文件
  ...
  if (s.ok()) {
    ...
    max_total_in_memory_state_ = 0;  //内存中的最大总大小，所有CF的buf数*buf大小，计算该值用来自动判断是否刷盘
    for (auto cfd : *versions_->GetColumnFamilySet()) {
    ...
    if (!immutable_db_options_.best_efforts_recovery) {
      s = env_->GetChildren(immutable_db_options_.wal_dir, &files_in_wal_dir);  //找出所有wal_dir 文件目录下文件
    }
    if (s.IsNotFound()) {
    ...
    for (const auto& file : files_in_wal_dir) { //遍历wal目录下的每个文件
      uint64_t number;
      FileType type;
      if (ParseFileName(file, &number, &type) && type == kWalFile) {  //解析文件名称，找出所有的WAL文件
        if (is_new_db) {  //新建的DB
          ...
          wal_files[number] =
              LogFileName(immutable_db_options_.wal_dir, number); //保存所有的 WAL文件名称
        }
      }
    }

    if (immutable_db_options_.track_and_verify_wals_in_manifest) {  //追踪并检查WAL 文件
      if (!immutable_db_options_.best_efforts_recovery) {
    ...
    if (!wal_files.empty()) {   //如果wal 文件数不为空，则检查是否有wal 有错
      if (error_if_wal_file_exists) { //入参传入为false
        return Status::Corruption(
            "The db was opened in readonly mode with error_if_wal_file_exists"
            "flag but a WAL file already exists");
      } else if (error_if_data_exists_in_wals) { //入参传入为false
    ...
    if (!wal_files.empty()) {
      ...
      wals.reserve(wal_files.size());   //reserve为vector预留空间，空间大小为 wal文件数的大小
      for (const auto& wal_file : wal_files) {
        wals.push_back(wal_file.first);
      }
      std::sort(wals.begin(), wals.end());  //取出所有wal文件名，并排序

      bool corrupted_wal_found = false;
      s = RecoverLogFiles(wals, &next_sequence, read_only,
                          &corrupted_wal_found);        //recover log文件信息，其中创建了memtable
      if (corrupted_wal_found && recovered_seq != nullptr) {
        *recovered_seq = next_sequence;   //更新 recovered_seq
      }
      if (!s.ok()) {  //清除memtable，如果recover失败
        // Clear memtables if recovery failed
        for (auto cfd : *versions_->GetColumnFamilySet()) {
          cfd->CreateNewMemtable(*cfd->GetLatestMutableCFOptions(),
                                 kMaxSequenceNumber); //如果recover log文件失败，也需要创建新的memtable
        }
      }
    }
  }

  if (read_only) {                  //如果是只读打开,需要更新OPTION文件，OPTION文件中保存了rocksdb的配置信息
...
Status DBImpl::PersistentStatsProcessFormatVersion() {
  ...
  mutex_.Unlock();
  if (persistent_stats_cfd_exists_) {    //默认就为 true
...
Status DBImpl::RecoverLogFiles(const std::vector<uint64_t>& wal_numbers,
  ...
  for (auto wal_number : wal_numbers) {
    ...
    // Open the log file
    std::string fname = LogFileName(immutable_db_options_.wal_dir, wal_number);   //找到log文件名称
    ...
    auto logFileDropped = [this, &fname]() {
      uint64_t bytes;
      if (env_->GetFileSize(fname, &bytes).ok()) {  //获取log文件大小
    ...
    log::Reader reader(immutable_db_options_.info_log, std::move(file_reader),
                       &reporter, true /*checksum*/, wal_number); //log文件的reader
    ...
    while (!stop_replay_by_wal_filter &&
           reader.ReadRecord(&record, &scratch,   //读log文件的record信息
                             immutable_db_options_.wal_recovery_mode) &&
           status.ok()) {
      ...
      status = WriteBatchInternal::InsertInto(                                //写入cf的memtable
      ...
      if (has_valid_writes && !read_only) {
        ...
        while ((cfd = flush_scheduler_.TakeNextColumnFamily()) != nullptr) {
          ...
          status = WriteLevel0TableForRecovery(job_id, cfd, cfd->mem(), edit);  //写L0文件
          if (!status.ok()) {
...
//五参Open函数，kvrocks创建数据库调用此Open函数
//DB::Open(options, config_->db_dir, column_families, &cf_handles_, &db_) //db_为上层封装的storage.db_, db_dir为DIR/db
Status DB::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  return DBImpl::Open(db_options, dbname, column_families, handles, dbptr,
                      !kSeqPerBatch, kBatchPerTxn); //false，true
}
...
Status DBImpl::Open(const DBOptions& db_options, const std::string& dbname,
                    const std::vector<ColumnFamilyDescriptor>& column_families,
                    std::vector<ColumnFamilyHandle*>* handles, DB** dbptr,
                    const bool seq_per_batch, const bool batch_per_txn) { //seq_per_batch = false、batch_per_txn = true
  Status s = ValidateOptionsByTable(db_options, column_families); //options检查
  if (!s.ok()) {
    return s;
  }

  s = ValidateOptions(db_options, column_families); //optins再检查，此步骤是否有冗余部分？
  if (!s.ok()) {
    return s;
  }
  ...
  *dbptr = nullptr;
  ...
  for (auto cf : column_families) {
    max_write_buffer_size =
        std::max(max_write_buffer_size, cf.options.write_buffer_size);  //所有选项里最大的write_buffer_size
  }

  DBImpl* impl = new DBImpl(db_options, dbname, seq_per_batch, batch_per_txn);  //创建 DBImpl 对象,也就是DB对象；seq_per_batch=false, batch_per_txn=true
  s = impl->env_->CreateDirIfMissing(impl->immutable_db_options_.wal_dir); //创建db的wal路径
    //创建有关的文件夹等
  if (s.ok()) {
  ...
  impl->wal_in_db_path_ = IsWalDirSameAsDBPath(&impl->immutable_db_options_); //记录wal路径与db路径是否相同

  impl->mutex_.Lock();
  // Handles create_if_missing, error_if_exists
  uint64_t recovered_seq(kMaxSequenceNumber);
  s = impl->Recover(column_families, false, false, false, &recovered_seq);  //从磁盘恢复数据，包括Current、manifest、log文件信息的recover
  if (s.ok()) {
    uint64_t new_log_number = impl->versions_->NewFileNumber(); //生成新 wal文件号
    log::Writer* new_log = nullptr;
    const size_t preallocate_block_size =
        impl->GetWalPreallocateBlockSize(max_write_buffer_size);  //获取预分配的buffer大小，此处用配置信息做一个限制，防止用户设置过大；在写wal文件时使用 AddRecord->Append函数
    s = impl->CreateWAL(new_log_number, 0 /*recycle_log_number*/,
                        preallocate_block_size, &new_log);  //创建新的 log文件writer，保存在 new_log
    if (s.ok()) {
      InstrumentedMutexLock wl(&impl->log_write_mutex_);
      impl->logfile_number_ = new_log_number;   //创建成功记录 wal文件号
      assert(new_log != nullptr);
      assert(impl->logs_.empty());
      impl->logs_.emplace_back(new_log_number, new_log);
    }

    if (s.ok()) {   //设置CF的handles
      // set column family handles
      for (auto cf : column_families) {
        auto cfd = impl->versions_->GetColumnFamilySet()->GetColumnFamily(cf.name);
        if (cfd != nullptr) { //CF存在，则直接创建 handles，存放到入参 handles
          handles->push_back(
              new ColumnFamilyHandleImpl(cfd, impl, &impl->mutex_));
          impl->NewThreadStatusCfInfo(cfd);
        }
        else {  //CF不存在，则创建
          if (db_options.create_missing_column_families) {  //kvrocks初始化设置为InitOptions() true
            // missing column family, create it
            ColumnFamilyHandle* handle;
            impl->mutex_.Unlock();
            s = impl->CreateColumnFamily(cf.options, cf.name, &handle);   //创建CF及其handles，CFhandles的理解？
            impl->mutex_.Lock();
            if (s.ok()) {
              handles->push_back(handle); //存放到入参 handles，是一个出参，传递给上层DB保存，如kvrocks
            } else {
              break;
            }
          }
    ...
    if (s.ok()) {   //superversion ？包含memtable、Immutable memtable、current version的增强版version
    ...
    if (s.ok()) {
      ...
      if (recovered_seq != kMaxSequenceNumber) {
        ...
        s = impl->WriteToWAL(empty_batch, log_writer, &log_used, &log_size);    //伪写wal文件，更新seq，作用？
        if (s.ok()) {
  ...
  if (s.ok() && impl->immutable_db_options_.persist_stats_to_disk) {  //持久化stats信息到磁盘，stats信息是哪些信息？
  ...
  if (s.ok()) {
    for (auto cfd : *impl->versions_->GetColumnFamilySet()) {
      if (cfd->ioptions()->compaction_style == kCompactionStyleFIFO) {
        ...
        for (int i = 1; i < vstorage->num_levels(); ++i) {  //检查L1及以上的level是否有文件，只有L0有文件时才能 FIFO compaction
  ...
  TEST_SYNC_POINT("DBImpl::Open:Opened");   //到此，DB创建结束


  Status persist_options_status;
  if (s.ok()) {   //将rocksdb配置信息写到磁盘，配置文件示例：dbdir/db/OPTIONS-000021
    // Persist RocksDB Options before scheduling the compaction.
    // The WriteOptionsFile() will release and lock the mutex internally.
    persist_options_status = impl->WriteOptionsFile(        //写操作
...
//ROCKSDB_LITE宏应用在移动设备场景，该宏禁止了一些消耗磁盘空间的功能，保持二进制精简
// 说明：https://code.aliyun.com/ckeyer/rocksdb/blob/db320b1b82e3c8458fdebf7989a280adaf0f1950/ROCKSDB_LITE.md
#ifndef ROCKSDB_LITE
  auto sfm = static_cast<SstFileManagerImpl*>(
      impl->immutable_db_options_.sst_file_manager.get());  //SstFileManager用来跟踪sst文件，并控制其删除速率；get方法获取shared_ptr的原始指针，赋值sfm
  if (s.ok() && sfm) {    //如果设置了sfm
    // Set Statistics ptr for SstFileManager to dump the stats of
    // DeleteScheduler.
    sfm->SetStatisticsPtr(impl->immutable_db_options_.statistics);    //设置统计信息，记录sst删除操作
    ...
    impl->mutex_.Lock();
    impl->versions_->GetLiveFilesMetaData(&metadata); //获取文件的meta信息
    impl->mutex_.Unlock();
    ...
    for (auto& path : paths) {
      ...
      for (auto& file_name : existing_files) {
        ...
        if (ParseFileName(file_name, &file_number, &file_type) &&
            file_type == kTableFile) {    //找到sst文件，文件类型 kTableFile
          ...
          if (known_file_sizes.count(file_name)) {
            ...
            sfm->OnAddFile(file_path, known_file_sizes.at(file_name),
                           /* compaction */ false)
                .PermitUncheckedError();    //添加一个文件到 sst_file_manager ， sst_file_manager的组织形式？？
          } else {
    ...
    sfm->ReserveDiskBuffer(max_write_buffer_size,
                           impl->immutable_db_options_.db_paths[0].path); //预留磁盘max_write_buffer_size大小的空间，以备在磁盘空间不足时候，至少还能写一个memtable内容
  }
  ...
  if (s.ok()) {
    //注册周期任务，具体哪些周期性任务？如果运作？
    //哪些任务：注册dumpstats、persiststats、flushinfolog三个任务，三个任务都来源于DBImpl实例；
    //如何运作？通过timer类来运行，Add()方法将任务添加到 timer.heap_，通过thread_.reset()来运行timer::run()，timer::run()中执行具体的任务函数；
