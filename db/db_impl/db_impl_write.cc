...
Status DBImpl::Write(const WriteOptions& write_options, WriteBatch* my_batch) {
  //write_options为默认WriteOptions
  return WriteImpl(write_options, my_batch, nullptr, nullptr);    //WriteImpl定义是9参的函数，此处4参调用？？
}
...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  //配置参数信息检查
  if (write_options.sync && write_options.disableWAL) { //sync==false
  ...
  if (write_options.low_pri) {  //low_pri==false
  ...
  if (two_write_queues_ && disable_memtable) {  //双写队列+memtable disable 则只写WAL
    ...
    return WriteImplWALOnly(&nonmem_write_thread_, write_options, my_batch,
                            ...
                            kDontPublishLastSeq, disable_memtable); //写流程总体与 WriteImpl一致
  }

  if (immutable_db_options_.unordered_write) {    //无序写？？
    ...
    Status status = WriteImplWALOnly(
        ...
        kDoPublishLastSeq, disable_memtable);   //写WAL log文件
    ...
    if (!disable_memtable) {
      ...
      status = UnorderedWriteMemtable(write_options, my_batch, callback,
                                      log_ref, seq, sub_batch_cnt);   //无序写 memtable，seq来源于WriteImplWALOnly；无序？？
    }
    return status;
  }

  if (immutable_db_options_.enable_pipelined_write) {   //是否启用了pipeline_write，pipelineWrite是将WAL和memtable分开写，提升性能；
  ...
  WriteThread::Writer w(write_options, my_batch, callback, log_ref,
                        disable_memtable, batch_cnt, pre_release_callback); //建立写操作，writer中带着要写的batch
  ...
  write_thread_.JoinBatchGroup(&w);   //将writer加入写队列，writer带有要写的batch; 更新w.state，如果是leader则直接返回，如果不是则阻塞等待

    //分三种情况进行写操作:
    // STATE_PARALLEL_MEMTABLE_WRITE
    // STATE_COMPLETE
    // STATE_GROUP_LEADER
  if (w.state == WriteThread::STATE_PARALLEL_MEMTABLE_WRITER) {    //1、并发写memtable，多线程写入
  ...
  if (w.state == WriteThread::STATE_COMPLETED) {    // 2、状态为完成则直接返回，多为followers被唤醒后的状态
    if (log_used != nullptr) {
      *log_used = w.log_used;
    }
    if (seq_used != nullptr) {
  ...
  assert(w.state == WriteThread::STATE_GROUP_LEADER);     // 3、如果是写group的leader，则进行正常流程wal、memtable的写入
  Status status;
  ...
  mutex_.Lock();  //多线程写，lockDB

  bool need_log_sync = write_options.sync;    //sync为false
  ...
  if (!two_write_queues_ || !disable_memtable) {
    ...
    status = PreprocessWrite(write_options, &need_log_sync, &write_context);    //写预处理，具体处理哪些内容？？
    if (!two_write_queues_) {
      // Assign it after ::PreprocessWrite since the sequence might advance
      // inside it by WriteRecoverableState
      last_sequence = versions_->LastSequence();    //获取之前的sequence
    }
  ...
  last_batch_group_size_ =  write_thread_.EnterAsBatchGroupLeader(&w, &write_group);  //3.1、写wal文件，获取batch group组合size

  IOStatus io_s;
  if (status.ok()) {
    // Rules for when we can update the memtable concurrently（同时地）
    ...
    bool parallel = immutable_db_options_.allow_concurrent_memtable_write &&
                    write_group.size > 1;   //允许同时写、并且group中不止一个writer时为true
    size_t total_count = 0;
    size_t valid_batches = 0;
    size_t total_byte_size = 0;
    size_t pre_release_callback_cnt = 0;
    for (auto* writer : write_group) {  //依次遍历整个wirte_group中的wirter，遍历其中的WriteBatch，获取WriteBatch count或size
      if (writer->CheckCallback(this)) {
        valid_batches += writer->batch_cnt;
        if (writer->ShouldWriteToMemtable()) {
          total_count += WriteBatchInternal::Count(writer->batch);  //获取batch数据的数量，注意不是大小，获得本次seq需要增加的总count
          parallel = parallel && !writer->batch->HasMerge();
        }
        total_byte_size = WriteBatchInternal::AppendedByteSize(
            total_byte_size, WriteBatchInternal::ByteSize(writer->batch));  //获取batch总字节大小
    ...
    size_t seq_inc = seq_per_batch_ ? valid_batches : total_count;  //sequence新增的count
    ...
    RecordInHistogram(stats_, BYTES_PER_WRITE, total_byte_size);  //statistics信息记录
    ...
    if (!two_write_queues_) {
      if (status.ok() && !write_options.disableWAL) {   //写wal
        PERF_TIMER_GUARD(write_wal_time);
        io_s = WriteToWAL(write_group, log_writer, log_used, need_log_sync,
                          need_log_dir_sync, last_sequence + 1);  //DB sequence+1
      }
    } else {
      if (status.ok() && !write_options.disableWAL) {
        ...
        last_sequence = versions_->FetchAddLastAllocatedSequence(seq_inc);    //increase sequence；seq_inc是sequence的增量，是写入batch的大小
      }
    }
    status = io_s;
    assert(last_sequence != kMaxSequenceNumber);
    const SequenceNumber current_sequence = last_sequence + 1;  //记录当前写入将从哪个sequence值开始
    last_sequence += seq_inc; //记录上一次写入后的sequence值,sequence增加，后面会反写回version的sequ_
    ...
    if (status.ok()) {
      ...
      for (auto* writer : write_group) {
        ...
        writer->sequence = next_sequence;   //更新每个writer的sequence
        if (writer->pre_release_callback) {
        ...
        if (seq_per_batch_) {
          ...
          next_sequence += WriteBatchInternal::Count(writer->batch);  //seq的增长是batch的count，也就是batch.put的次数
        }
      }
    }
    ...
    if (status.ok()) {
      ...
      if (!parallel) {    //只有一个写
        ...
        w.status = WriteBatchInternal::InsertInto(  //写memtable，更新sequence
      ...
      else {  //同时多个写，允许并行写、且group中的writer大于1个
        write_group.last_sequence = last_sequence;
        write_thread_.LaunchParallelMemTableWriters(&write_group);    //3.3、唤醒等待线程，再写memtables，更新followers状态STATE_PARALLEL_MEMTABLE_WRITER
        in_parallel_group = true;

        // Each parallel follower is doing each own writes. The leader should
        // also do its own.
        if (w.ShouldWriteToMemtable()) {  //w是每个并发写创建的writer
          ColumnFamilyMemTablesImpl column_family_memtables(
              versions_->GetColumnFamilySet());
          assert(w.sequence == current_sequence);
          w.status = WriteBatchInternal::InsertInto(    //写memtables
              &w, w.sequence, &column_family_memtables, &flush_scheduler_,
              &trim_history_scheduler_,
  ...
  if (need_log_sync) {  //写完wal，进行wal刷盘
    mutex_.Lock();
    if (status.ok()) {
  ...
  if (in_parallel_group) {
    ...
    should_exit_batch_group = write_thread_.CompleteParallelMemTableWriter(&w);//3.4、完成memtable同步写入，等待所有writer完成，如果不是最后一个writer完成此处返回false
  }
  if (should_exit_batch_group) {  //如果write group退出；此处为真表明是所有writer都完成
    if (status.ok()) {
      // Note: if we are to resume after non-OK statuses we need to revisit how
      // we reacts to non-OK statuses here.
      versions_->SetLastSequence(last_sequence);  //更新本次写后的sequence
    }
    MemTableInsertStatusCheck(w.status);
    write_thread_.ExitAsBatchGroupLeader(write_group, status); //group leader退出，定出下一个leader、更新本次group中的writer为STATE_COMPLETED
  }
...
//理解： https://kernelmaker.github.io/Rocksdb_pipeline_write
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  WriteThread::Writer w(write_options, my_batch, callback, log_ref,
                        disable_memtable);
                        //1、入队列newest writer （不是leader则阻塞等待被唤醒）
  ...
  if (w.state == WriteThread::STATE_GROUP_LEADER) {
    ...
    //2、如果自己是leader，打包队列中的writer到wal write group
...
//3、写wal
    ...
    //4、将wal write group连接newest_memtable_writer_队列中，然后选择newest writer队列中的新的leader并唤醒它（不是则阻塞等待）
  ...
  if (w.state == WriteThread::STATE_MEMTABLE_WRITER_LEADER) {
    ...
    //5、如果是newest memtable writer_队列的leader，则打包队列中的writer到memtable write group中
...
//7、同样，在完成后选择队列中的新leader并唤醒它，更新memtable write_group中所有的writer的state唤醒followers
...
Status DBImpl::UnorderedWriteMemtable(const WriteOptions& write_options,
  ...
  if (w.CheckCallback(this) && w.ShouldWriteToMemtable()) {
    ...
    w.status = WriteBatchInternal::InsertInto(        //写入memtable
        &w, w.sequence, &column_family_memtables, &flush_scheduler_,
        &trim_history_scheduler_, write_options.ignore_missing_column_families,
        0 /*log_number*/, this, true /*concurrent_memtable_writes*/,
...
Status DBImpl::WriteImplWALOnly(
  ...
  if (w.state == WriteThread::STATE_COMPLETED) {    //group都写完成
    if (log_used != nullptr) {
      *log_used = w.log_used;
    }
    if (seq_used != nullptr) {    //更新sequence
      *seq_used = w.sequence;
    }
    return w.FinalStatus();
  }
...
Status DBImpl::PreprocessWrite(const WriteOptions& write_options,
  ...
  if (error_handler_.IsDBStopped()) {   //检查DB状态
    status = error_handler_.GetBGError();
  }
  ...
  if (UNLIKELY(status.ok() && !single_column_family_mode_ &&
    ...
    status = SwitchWAL(write_context);  //切换新的log文件
  }
  ...
  *write_with_wal = 0;
  ...
  if (write_group.size == 1 && !leader->CallbackFailed() &&
    ...
    *write_with_wal = 1;
    ...
    // interface
    merged_batch = tmp_batch; //给merged_batch创建了头部
    for (auto writer : write_group) {
      if (!writer->CallbackFailed()) {
        Status s = WriteBatchInternal::Append(merged_batch, writer->batch,
                                              /*WAL_only*/ true); //将writer中的batch逐条append，append时去掉头部，只保留内容
...
IOStatus DBImpl::WriteToWAL(const WriteBatch& merged_batch,
  ...
  Slice log_entry = WriteBatchInternal::Contents(&merged_batch);  //获取req_，req_按Slice类型返回
  *log_size = log_entry.size();
  ...
  IOStatus io_s = log_writer->AddRecord(log_entry); //将batch的req_内容写入wal文件
...
IOStatus DBImpl::WriteToWAL(const WriteThread::WriteGroup& write_group,
  ...
  WriteBatch* merged_batch = MergeBatch(write_group, &tmp_batch_,
                                        &write_with_wal, &to_be_cached_state);  //构造mergeBatch，将writer中的batch去掉头部都编码string存入merged_batch.req_
  ...
  WriteBatchInternal::SetSequence(merged_batch, sequence);  //将seq依次编入merged_batch的req_中，在batch.req_[0]头部
...
Status DBImpl::SwitchWAL(WriteContext* write_context) {
  ...
  for (const auto cfd : cfds) {
    cfd->Ref();
    status = SwitchMemtable(cfd, write_context);  //切换memtable
...
size_t DBImpl::GetWalPreallocateBlockSize(uint64_t write_buffer_size) const {
  ...
  if (mutable_db_options_.max_total_wal_size > 0) {
    bsize = std::min<size_t>(bsize, static_cast<size_t>(mutable_db_options_.max_total_wal_size));
  }
  ...
  if (immutable_db_options_.write_buffer_manager &&
    ...
    bsize = std::min<size_t>(bsize, immutable_db_options_.write_buffer_manager->buffer_size());
  }
