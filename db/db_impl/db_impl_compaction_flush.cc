...
Status DBImpl::BackgroundFlush(bool* made_progress, JobContext* job_context,
  ...
  *reason = FlushReason::kOthers;
  ...
  while (!flush_queue_.empty()) { // flush_queue_记录待flush的CF的信息，由DBImpl::SchedulePendingFlush注册到其中
...
void DBImpl::BackgroundCallCompaction(PrepickedCompaction* prepicked_compaction,
                       ...
                       immutable_db_options_.info_log.get());
  {
    ...
    if (s.IsBusy()) {
      ...
      env_->SleepForMicroseconds(1000000);
      mutex_.Lock();
    } else if (s.IsManualCompactionPaused()) {  //manual compaction会pause自动compaction？
...
Status DBImpl::BackgroundCompaction(bool* made_progress,
  ...
  ManualCompactionState* manual_compaction =
      prepicked_compaction == nullptr
          ? nullptr
          : prepicked_compaction->manual_compaction_state;  //prepicked compaction是manualcompaction，由调用RunManualCompaction接口注册
  *made_progress = false;
  mutex_.AssertHeld();
  ...
  CompactionJobStats compaction_job_stats;
  Status status;
  if (!error_handler_.IsBGWorkStopped()) {  //后台任务停止，则查找停止原因
    if (shutting_down_.load(std::memory_order_acquire)) {
      status = Status::ShutdownInProgress();
    } else if (is_manual &&
               manual_compaction_paused_.load(std::memory_order_acquire) > 0) {
  ...
  bool sfm_reserved_compact_space = false;
  if (is_manual) {          // manual compaction任务被选择
    ...
    if (!c) { //如果任务为空，则打印日志即可
      ...
      if (!enough_room) { //空间不足则不进行该compaction，并返回空间不足错误
      ...
      } else {  //记录manual compaction的执行，具体compaction在RunManualCompaction执行
        ROCKS_LOG_BUFFER(
            log_buffer,
  ...
  } else if (!is_prepicked && !compaction_queue_.empty()) {     // 自动compaction任务被选择
    if (HasExclusiveManualCompaction()) { //判断是否有排他manual compaction在执行，如有则不执行并保持本compaction任务
    ...
    auto cfd = PickCompactionFromQueue(&task_token, log_buffer);  //取出 compaction CF
    if (cfd == nullptr) { //thread数达到limit，先不执行
    ...
    if (!mutable_cf_options->disable_auto_compactions && !cfd->IsDropped()) { //自动compaction
      // NOTE: try to avoid unnecessary copy of MutableCFOptions if
      // compaction is not necessary. Need to make sure mutex is held
      // until we make a copy in the following code
      TEST_SYNC_POINT("DBImpl::BackgroundCompaction():BeforePickCompaction");
      c.reset(cfd->PickCompaction(*mutable_cf_options, mutable_db_options_,
                                  log_buffer)); //设置compaction
  ...
  if (!c) {
    ...
    for (const auto& f : *c->inputs(0)) {
      c->edit()->DeleteFile(c->level(), f->fd.GetNumber()); //删除compaction的文件
    }
    ...
    *made_progress = true;
    ...
    env_->Schedule(&DBImpl::BGWorkBottomCompaction, ca, Env::Priority::BOTTOM,
                   this, &DBImpl::UnscheduleCompactionCallback);  //注册BGWorkBottomCompaction任务
  } else {
    TEST_SYNC_POINT_CALLBACK("DBImpl::BackgroundCompaction:BeforeCompaction",
                             c->column_family_data());
    ...
    compaction_job.Prepare(); // 1. 为compaction执行做准备，如果需要subcompaction则为每个subcompaction拆分边界
    ...
    // Should handle erorr?
    compaction_job.Run().PermitUncheckedError();  // 2. 执行compaction，compaction拆分成subcompaction并发的执行
    TEST_SYNC_POINT("DBImpl::BackgroundCompaction:NonTrivial:AfterRun");
    mutex_.Lock();

    status = compaction_job.Install(*c->mutable_cf_options());  //3. compaction执行完成后，注册文件信息到version
  ...
  if (c != nullptr) {
    ...
    *made_progress = true;
    ...
    NotifyOnCompactionCompleted(c->column_family_data(), c.get(), status,
                                compaction_job_stats, job_context->job_id); //compaction结束
  }
