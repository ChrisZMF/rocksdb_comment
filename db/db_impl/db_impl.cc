...
void DBImpl::StartPeriodicWorkScheduler() {
  ...
  {
    InstrumentedMutexLock l(&mutex_);
    periodic_work_scheduler_ = PeriodicWorkScheduler::Default();  //创建timer
    TEST_SYNC_POINT_CALLBACK("DBImpl::StartPeriodicWorkScheduler:Init",
                             &periodic_work_scheduler_);
  }

  periodic_work_scheduler_->Register( //运行timer.start()， 并注册周期任务
...
void DBImpl::PersistStats() {
  ...
  if (immutable_db_options_.persist_stats_to_disk) {
    ...
    if (s.ok()) {
      ...
      wo.no_slowdown = true;
      wo.sync = false;
      s = Write(wo, &batch);    //信息写入磁盘， DBImpl::Write()
    }
    if (!s.ok()) {
      ROCKS_LOG_INFO(immutable_db_options_.info_log,
...
InternalIterator* DBImpl::NewInternalIterator(const ReadOptions& read_options,
  ...
  merge_iter_builder.AddIterator(
      super_version->mem->NewIterator(read_options, arena));  //创建memtable的iter，放在first_iter
  ...
  if (s.ok()) {
    super_version->imm->AddIterators(read_options, &merge_iter_builder);  //创建immemtable的iter,有多个immemtable依次创建
    if (!read_options.ignore_range_deletions) {
  ...
  if (s.ok()) {
    ...
    if (read_options.read_tier != kMemtableTier) {  //创建sst文件的L0-Ln的iter，逐层创建；分L0层与LN层，L0层创建table_cache的iter，Ln层创建LevelIterator
      super_version->current->AddIterators(read_options, file_options_,
                                           &merge_iter_builder, range_del_agg,
                                           allow_unprepared_value);
    }
    internal_iter = merge_iter_builder.Finish();  //将merge_iter返回给internal_iter，merge_iter中汇总了mem、immem、L0-Ln的所有iters
    IterState* cleanup =
...
//DB::Get实现,如，kvrocks string类型get调用此接口
Status DBImpl::Get(const ReadOptions& read_options,
                   ColumnFamilyHandle* column_family, const Slice& key,
                   PinnableSlice* value) {  //kvrocks传入value为string类型的buf，在db.h中赋值给PinnableSlice.buf_=buf
  return Get(read_options, column_family, key, value, /*timestamp=*/nullptr);
}

Status DBImpl::Get(const ReadOptions& read_options, //read_options里带了当前的snapshot
                   ColumnFamilyHandle* column_family, const Slice& key, //key为传入的要查找的key
                   PinnableSlice* value, std::string* timestamp) {
  GetImplOptions get_impl_options;
  get_impl_options.column_family = column_family; //调用接口传入值
  get_impl_options.value = value; //value是出参
...
//Get的底层实现
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key, //read_options里带来当前snapshot
                       GetImplOptions& get_impl_options) {
  ...
  GetWithTimestampReadCallback read_cb(0);  // Will call Refresh max_visible_seq_
  ...
  auto cfh = static_cast_with_check<ColumnFamilyHandleImpl>(
      get_impl_options.column_family);  //cf handle 调用db接口入参传入指定的cf
  auto cfd = cfh->cfd();  //cf data； cf_data结构元素指向一个superversion，sv指向了mem、imem、current等
  ...
  SuperVersion* sv = GetAndRefSuperVersion(cfd);  //1、获取当前的superversion

  TEST_SYNC_POINT("DBImpl::GetImpl:1");
  TEST_SYNC_POINT("DBImpl::GetImpl:2");

  SequenceNumber snapshot;  //snapshot是一个整数，实际就是sequence； 2、获取snapshot
  if (read_options.snapshot != nullptr) {
    if (get_impl_options.callback) { //callback默认为null
      // Already calculated based on read_options.snapshot
      snapshot = get_impl_options.callback->max_visible_seq();
    } else {
      snapshot =  //使用传入的snapshot
          reinterpret_cast<const SnapshotImpl*>(read_options.snapshot)->number_;  //number_即snapshot创建时填入的seq
    }
  }
  else
  {
  ...
  if (ts_sz > 0 && !get_impl_options.callback) {    //更新sequence
    read_cb.Refresh(snapshot);              //更新max_visible_seq_
  ...
  LookupKey lkey(key, snapshot, read_options.timestamp);    //3、创建lookupkey对象，准备查找
  ...
  if (!skip_memtable) { //读memtable
    // Get value associated with key
    if (get_impl_options.get_value) {   //默认为true
      //4、读取memtable
      //sv->mem即为memtable
      if (sv->mem->Get(lkey, get_impl_options.value->GetSelf(), timestamp, &s, //value->GetSelf()返回的buf_就是上层传入的buf
        ...
        //5、读Immutable memtable；与memtable读的区别在于，Immutable memtable是多个，组成一个list，依次读取
    ...
    else {    //当调用DB::GetMergeOperands时，会先设置get_value=false，再调用GetImpl()
  ...
  //读memtable、Immutable memtable失败
  //6、读current version，读sst文件
  if (!done) {
    ...
    sv->current->Get(
        ...
        get_impl_options.get_value ? get_impl_options.is_blob_index : nullptr,    //get_value默认为true，所以传入is_blob_index（默认为NULL)
  ...
  //7、get收尾工作
  {
    ...
    ReturnAndCleanupSuperVersion(cfd, sv);      //释放创建的superversion

    RecordTick(stats_, NUMBER_KEYS_READ); //记录log信息
    size_t size = 0;
    if (s.ok()) {
      if (get_impl_options.get_value) {
        size = get_impl_options.value->size();  //获取value的size，用于记录log
      }
      else {
    ...
    RecordInHistogram(stats_, BYTES_PER_READ, size);  //记录统计信息
  }
  return s;
}
...
Status DBImpl::CreateColumnFamily(const ColumnFamilyOptions& cf_options,
                                  const std::string& column_family_name, //cf_name
                                  ColumnFamilyHandle** handle) {
  assert(handle != nullptr);
  Status s = CreateColumnFamilyImpl(cf_options, column_family_name, handle);
  if (s.ok()) {
...
Iterator* DBImpl::NewIterator(const ReadOptions& read_options,
  ...
  if (read_options.tailing) { //构造数据库全量iter，并能跟踪DB新的写入
#ifdef ROCKSDB_LITE
    // not supported in lite version
    result = nullptr;

#else
    SuperVersion* sv = cfd->GetReferencedSuperVersion(this);  //获取当前的superversion
    auto iter = new ForwardIterator(this, read_options, cfd, sv,
                                    /* allow_unprepared_value */ true);
    result = NewDBIterator(
        env_, read_options, *cfd->ioptions(), sv->mutable_cf_options,
        cfd->user_comparator(), iter, sv->current, kMaxSequenceNumber,  //tailing模式则传入kMaxSequenceNumber作为endseq
...
ArenaWrappedDBIter* DBImpl::NewIteratorImpl(const ReadOptions& read_options,
                                            ...
                                            bool allow_refresh) {
  SuperVersion* sv = cfd->GetReferencedSuperVersion(this);  //cf对应的当前的sv
  ...
  if (snapshot == kMaxSequenceNumber) {
    ...
    // calls NewIterator().
    snapshot = versions_->LastSequence(); //从version拿到snapshot； CF、sv、version、snapshot的关系？
  ...
  InternalIterator* internal_iter = NewInternalIterator(
      ...
      /* allow_unprepared_value */ true); //InternalIterator中包含了mem、多个Immem、L0-Ln各层的iters，也就是整个DB的内容都能被iter到
  db_iter->SetIterUnderDBIter(internal_iter); //将internal_iter附给db_iter，db_iter主要就是一个DBIter，而DBIter主要就是保存
...
#ifndef ROCKSDB_LITE
Status DBImpl::GetUpdatesSince( //从seq处生成迭代器，迭代wal中的内容；kvrocks增量复制处应用
...
Status DBImpl::IngestExternalFiles(
  ...
  TEST_SYNC_POINT("DBImpl::AddFile:Start");
  {
    ...
    if (status.ok()) {
      for (size_t i = 0; i != num_cfs; ++i) {
        status = ingestion_jobs[i].Run(); //执行注入文件的job
        if (!status.ok()) {
          break;
        }
      }
