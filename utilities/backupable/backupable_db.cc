...
Status BackupEngineImpl::CreateNewBackupWithMetadata(
    ...
    const std::string& app_metadata) {  //app_metadata是timestamp
  assert(initialized_);
  assert(!read_only_);
  ...
  auto private_dir = GetAbsolutePath(GetPrivateFileRel(new_backup_id)); //找到backup的private绝对路径
  ...
  if (s.ok()) {
    ...
    s = GarbageCollect();   //清除private目录下的垃圾
  ...
  auto ret = backups_.insert(std::make_pair(
      new_backup_id, std::unique_ptr<BackupMeta>(new BackupMeta(
                         GetBackupMetaFile(new_backup_id, false /* tmp */), //meta/backupid 
                         GetBackupMetaFile(new_backup_id, true /* tmp */),  //meta/.backupid.tmp
                         &backuped_file_infos_, backup_env_))));
  ...
  auto start_backup = backup_env_->NowMicros(); //获取当前时间毫秒单位

  ROCKS_LOG_INFO(options_.info_log,
                 "Started the backup process -- creating backup %u",
                 new_backup_id);
  if (s.ok()) {
    s = backup_env_->CreateDir(private_dir);  //创建private目录
  }
  ...
  Status disabled = db->DisableFileDeletions(); //禁止db文件删除操作，生成backup文件
  if (s.ok()) {
    CheckpointImpl checkpoint(db);  //checkpoint对象
    ...
    s = checkpoint.CreateCustomCheckpoint(  //创建checkpoint，backup机制使用checkpoint来实现
        db_options,
        ...
        compare_checksum);
    if (s.ok()) {
      new_backup->SetSequenceNumber(sequence_number); //设置当前的sequence
    }
  }
  ...
  if (disabled.ok()) {  // If we successfully disabled file deletions
    db->EnableFileDeletions(false).PermitUncheckedError();  //已完成文件拷贝，重新开启文件删除功能，可以compact
  }
  auto backup_time = backup_env_->NowMicros() - start_backup; //backup的总时长，毫秒

  if (s.ok()) {
    // persist the backup metadata on the disk
    s = new_backup->StoreToFile(options_.sync); //meta信息刷盘
  }
  ...
  if (s.ok()) {
    backup_statistics_.IncrementNumberSuccessBackup();  //成功记录成功备份次数
  }
  if (!s.ok()) {
  ...
  latest_backup_id_ = new_backup_id;  //更新记录最新备份的id，及有效备份的id（可能备份失败）
...
//end CreateNewBackupWithMetadata
...
Status BackupEngineImpl::BackupMeta::StoreToFile(bool sync) {
  ...
  s = env_->NewWritableFile(meta_tmp_filename_, &backup_meta_file, env_options);//meta_tmp_filename_在前文已经生成，见new_backup
  if (!s.ok()) {
    return s;
  }

  std::ostringstream buf;
  buf << timestamp_ << "\n";  //meta内容：timestamp/seq/metadata+datastring/files size/files...
