...
Status CheckpointImpl::CreateCustomCheckpoint(
  ...
  *sequence_number = db_->GetLatestSequenceNumber();
  ...
  s = db_->GetLiveFiles(live_files, &manifest_file_size, flush_memtable); //获取live_files，通过version来获取当前的文件信息
  ...
  for (auto& live_file : live_files) {  //逐条处理manifest文件信息
    if (!s.ok()) {
      break;
    }
    uint64_t number;
    ...
    if (type != kTableFile) {
      ...
      live_table_files.push_back(make_pair(live_file, number)); //通过manifest文件找出table文件等信息
    }
  }
