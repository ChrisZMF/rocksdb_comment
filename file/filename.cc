...
Status SetIdentityFile(Env* env, const std::string& dbname,
  ...
  std::string id;
  if (db_id.empty()) {
    id = env->GenerateUniqueId(); //dbid为空，则生成唯一Id字串
  } else {
    id = db_id;
  }
  assert(!id.empty());
  // Reserve the filename dbname/000000.dbtmp for the temporary identity file
  std::string tmp = TempFileName(dbname, 0);  //创建临时文件写dbid信息
  Status s = WriteStringToFile(env, id, tmp, true);
  if (s.ok()) {
    s = env->RenameFile(tmp, IdentityFileName(dbname)); //修改文件名称为 dbname/IDENTITY
  }
  if (!s.ok()) {
