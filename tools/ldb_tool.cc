...
int LDBCommandRunner::RunCommand(
  ...
  if (argc <= 2) {  //参数个数太少，打印help信息
    if (argc <= 1) {  //参数个数<=1，即只有一个可执行文件名无参数
  ...
  LDBCommand* cmdObj = LDBCommand::InitFromCmdLineArgs(
      argc, argv, options, ldb_options, column_families); //初始化并找到要执行的命令对象
  if (cmdObj == nullptr) {
  ...
  if (!cmdObj->ValidateCmdLineOptions()) {  //参数检查
    return 1;
  }

  cmdObj->Run();  //执行命令；统一为： LDBCommand::Run()
