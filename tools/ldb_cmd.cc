...
LDBCommand* LDBCommand::InitFromCmdLineArgs(
    ...
    const std::vector<ColumnFamilyDescriptor>* column_families) { //除argc、argv来自cmdline，其他参数都为默认值
  std::vector<std::string> args;
  for (int i = 1; i < argc; i++) {  //汇总所有args
...
LDBCommand* LDBCommand::InitFromCmdLineArgs(
  ...
  //命令实例：./ldb dump_wal --db=%s --walfile=%s --header
  for (const auto& arg : args) {  //parse命令行参数
    if (arg[0] == '-' && arg[1] == '-'){
      ...
      cmdTokens.push_back(arg); //获取命令名及参数列表，非'--'开头的options
    }
  }
  ...
  LDBCommand* command = selector(parsed_params);  //selector即SelectCommand

  if (command) {
    command->SetDBOptions(options); //options赋值给command的options_
...
LDBCommand* LDBCommand::SelectCommand(const ParsedParams& parsed_params) {
  if (parsed_params.cmd == GetCommand::Name()) {
    ...
    return new WALDumperCommand(parsed_params.cmd_params, //命令参数、命令行选项options、flags
                                parsed_params.option_map, parsed_params.flags);
...
void LDBCommand::Run() {
  ...
  if (!options_.env || options_.env == Env::Default()) {  //创建env对象
    Env* env = Env::Default();
  ...
  if (db_ == nullptr && !NoDBOpen()) {  //如需要打开db，则opendb
    OpenDB();
...
void DumpWalFile(Options options, std::string wal_file, bool print_header,
                 bool print_values, bool is_write_committed,
                 LDBCommandExecuteResult* exec_state) {
  Env* env = options.env;   // 使用Env::Default()
...
WALDumperCommand::WALDumperCommand(
    const std::vector<std::string>& /*params*/,
    const std::map<std::string, std::string>& options,  //此处options是命令行执行传入的options，由'--'打头，如：./ldb dump_wal --db=%s --walfile=%s --header
    const std::vector<std::string>& flags)
    : LDBCommand(options, flags, true,
  ...
  std::map<std::string, std::string>::const_iterator itr =
      options.find(ARG_WAL_FILE); //找到--walfile，以得到wal文件名
  if (itr != options.end()) {
    wal_file_ = itr->second;  //wal文件名，由命令行传入的
  }
