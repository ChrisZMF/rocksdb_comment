...
class MergingIterator : public InternalIterator {
 public:
  ...
  void SeekToFirst() override {
    ClearHeaps();
    status_ = Status::OK();
    for (auto& child : children_) { //children_中存放了mem、多个immem、sst文件L0-Ln的所有iters，此处依次遍历
      child.SeekToFirst();  //使用各自iter的实现来seekToFirst
      AddToMinHeapOrCheckStatus(&child);  //将child加入到minHeap_中，current_再从中取值
    }
    direction_ = kForward;
    current_ = CurrentForward();  //current_再从minHeap_中取值
  }
  ...
  void SeekToLast() override {
    ...
    status_ = Status::OK();
    for (auto& child : children_) {
      child.SeekToLast(); //child为各个模块的iter，调用其特定的接口
      AddToMaxHeapOrCheckStatus(&child);//将child加入到maxHeap中，current_再从中取值
    }
    direction_ = kReverse;
...
void MergeIteratorBuilder::AddIterator(InternalIterator* iter) {
  if (!use_merging_iter && first_iter != nullptr) {
    merge_iter->AddIterator(first_iter);  //2、第二次调用时
    use_merging_iter = true;  //只在此赋值true
    first_iter = nullptr;
  }
  if (use_merging_iter) {
    merge_iter->AddIterator(iter);  //3、第三次及以后的调用
  } else {
    first_iter = iter;  //1、接口初次调用时候赋值给first_iter
  }
}
